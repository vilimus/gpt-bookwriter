...
class Book():
  ...
  def save_to_html(self, filename=None, section_titles=True):
...
titles = output.split("\n")
titles = [titles[0].strip()] + [re.search("Chapter (\d+): (.*)", title).group(2) for title in titles[1:]]
...
for i, chapter in enumerate(book.chapters):
  ...
  titles = [titles[0].strip()] + [re.search(f"Section {i+1}\.(\d+): (.*)", title).group(2) for title in titles[1:]]
...
for i, chapter in enumerate(book.chapters):
  ...
  for j, section in enumerate(chapter.sections):
    ...
    prompt = f"We are writing a book about {topic}. The book has the following chapter titles:\n" + "\n".join([f"Chapter {i+1}: {chapter.title}" for i, chapter in enumerate(chapters)]) + f"\nWe want to generate a section for Chapter {i+1}: {chapter.title}, and nothing more. The chapter has the following sections:\n" + "\n".join([f"Section {i+1}.{j+1}: {section.title}" for i, chapter in enumerate(chapter.sections)]) + f"\nWe want to generate the text of Section {i+1}.{j+1}, and nothing more. Generate several lengthy paragraphs for the aforementioned section of the book. We want the sections to be verbose, and highly descriptive. Lets write in the style of Franz Kafka. Do not number the paragraphs, we just want the text itself:\n"
